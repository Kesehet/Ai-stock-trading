...
class RiskLimits:
    ...
    # Tiny accounts need some whole-share rounding room: a ₹64 share is already
    # ~5x a 2.5% allocation on ₹500. Eight times preserves those useful cheap
    # trades while rejecting MOSCHIP-like ~18x allocation jumps.
    max_whole_share_allocation_multiple: float = 8.0
...
class RiskEngine:
    ...
    def _delivery_cost_adjusted_reward_risk(
        ...
        *,
        ...
        entry_cost = ZERODHA_NSE_CASH_2026.charges(turnover=entry_turnover, side=Side.BUY, product=Product.DELIVERY)
        target_exit_cost = ZERODHA_NSE_CASH_2026.charges(turnover=target_turnover, side=Side.SELL, product=Product.DELIVERY, include_dp=True)
        stop_exit_cost = ZERODHA_NSE_CASH_2026.charges(turnover=stop_turnover, side=Side.SELL, product=Product.DELIVERY, include_dp=True)
    ...
    def evaluate(self, intent: TradeIntent, quote: Quote, portfolio: PortfolioSnapshot, now: datetime | None = None) -> RiskDecision:
        ...
        if intent.side == Side.BUY and intent.stop_price is not None and intent.stop_price >= quote.last_price:
            return RiskDecision(approved=False, reason="Buy stop must be below entry price")
        if intent.side == Side.BUY and intent.target_price is not None and intent.target_price <= quote.last_price:
            return RiskDecision(approved=False, reason="Buy target must be above entry price")
        if intent.side == Side.BUY and intent.stop_price is not None and intent.target_price is not None:
            downside = quote.last_price - intent.stop_price
            upside = intent.target_price - quote.last_price
            if (upside / downside if downside > 0 else 0.0) < self.limits.min_reward_risk:
                return RiskDecision(approved=False, reason="Expected reward does not justify defined downside risk")

        matching_positions = [p for p in portfolio.positions if p.symbol == intent.symbol and p.product == intent.product]
        held_quantity = sum(p.quantity for p in matching_positions)
        existing_cost = sum(p.quantity * p.average_price for p in matching_positions)
        current_value = held_quantity * quote.last_price
        if portfolio.open_positions >= self.limits.max_open_positions and intent.side == Side.BUY and held_quantity == 0:
        ...
        if intent.side == Side.SELL:
            ...
            desired_remaining = min(current_value, portfolio.equity * intent.target_allocation_pct)
            sell_notional = max(0.0, current_value - desired_remaining)
            quantity = held_quantity if intent.target_allocation_pct == 0 else floor(sell_notional / quote.last_price)
            if quantity <= 0:
                return RiskDecision(approved=False, reason="Position is already at or below requested remaining allocation")
            quantity = min(quantity, held_quantity)
        else:
            position_cap = portfolio.equity * self.limits.max_position_pct
            desired_notional = min(portfolio.equity * intent.target_allocation_pct, position_cap)
            if held_quantity == 0 and intent.stop_price is not None and desired_notional < quote.last_price <= position_cap and quote.last_price <= portfolio.cash:
                requested_notional = portfolio.equity * intent.target_allocation_pct
                max_granular_notional = requested_notional * self.limits.max_whole_share_allocation_multiple
                if requested_notional <= 0 or quote.last_price > max_granular_notional:
                    return RiskDecision(approved=False, reason="Minimum whole-share position materially exceeds requested allocation")
            ...
            if intent.stop_price is not None:
                ...
                if risk_per_share > 0:
                    ...
                    existing_defined_risk = sum(p.quantity * max(0.0, p.average_price - intent.stop_price) for p in matching_positions)
                    remaining_risk_budget = max(0.0, risk_budget - existing_defined_risk)
                    risk_fraction = risk_per_share / quote.last_price
                    risk_notional_cap = remaining_risk_budget / risk_fraction if risk_fraction > 0 else 0.0
            ...
            if quantity <= 0 and current_value >= desired_notional:
                return RiskDecision(approved=False, reason="Position is already at or above target allocation")

        if quantity <= 0:
            return RiskDecision(approved=False, reason="Insufficient capital or risk budget")
        if intent.side == Side.BUY and held_quantity == 0 and intent.product == Product.DELIVERY and intent.stop_price is not None and intent.target_price is not None:
            net_reward, _, cost_adjusted_reward_risk = self._delivery_cost_adjusted_reward_risk(quantity=quantity, entry_price=quote.last_price, stop_price=intent.stop_price, target_price=intent.target_price)
            if net_reward <= 0:
                return RiskDecision(approved=False, reason="Delivery target does not clear realistic charges and slippage")
            if cost_adjusted_reward_risk < self.limits.min_reward_risk:
                return RiskDecision(approved=False, reason="Cost-adjusted reward does not justify delivery downside risk")
        if intent.side == Side.BUY and held_quantity > 0:
            projected_quantity = held_quantity + quantity
            projected_average = (existing_cost + quantity * quote.last_price) / projected_quantity
            if intent.target_price is not None and intent.target_price <= projected_average:
                return RiskDecision(approved=False, reason="Target does not clear blended position cost")
            if intent.stop_price is not None and intent.target_price is not None and intent.stop_price < projected_average:
                blended_downside = projected_average - intent.stop_price
                blended_upside = intent.target_price - projected_average
                if (blended_upside / blended_downside if blended_downside > 0 else 0.0) < self.limits.min_reward_risk:
                    return RiskDecision(approved=False, reason="Expected reward does not justify blended position risk")
        plan = OrderPlan(intent_id=f"{intent.thesis_id}:{int(intent.decision_at.timestamp())}", symbol=intent.symbol, side=intent.side, product=intent.product, quantity=quantity, limit_price=quote.last_price, stop_price=intent.stop_price, target_price=intent.target_price)
