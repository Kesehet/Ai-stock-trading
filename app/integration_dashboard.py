...
def render_integrations_page(
    ...
    *,
    ...
    tested = {
        ...
        if isinstance(item, IntegrationCheck)
    }
    services = [
    ...
    for key, name, purpose, needs_config in services:
        ...
        if result is not None:
            ...
            latency = ""
        disabled = " disabled" if not admin_enabled else ""
        cards.append(
            f"""
...
<button type="submit" name="integration" value="{html.escape(key)}"{disabled}>Test {html.escape(name)}</button>
</section>
"""
        )
